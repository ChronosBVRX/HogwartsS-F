...
begin
  ...
  -- 1. MEJORA DE IA (Si el modo es AI)
  if v_duel.mode = 'ai' then
    ...
    if v_duel.player_two_hp < 35 and v_ai_rand < 0.6 then
      ...
      v_p2_spell_key := 'accio'; -- Cargar energía obligatoriamente
    elsif v_duel.player_one_energy < 2 and v_ai_rand < 0.7 then
      v_p2_spell_key := 'stupefy'; -- Castigar falta de energía del jugador
    elsif v_ai_rand < 0.3 then
      v_p2_spell_key := 'protego'; -- Táctico defensivo
    elsif v_ai_rand < 0.6 then
      v_p2_spell_key := 'incendio'; -- Ataque balanceado
    elsif v_ai_rand < 0.8 then
      v_p2_spell_key := 'petrificus'; -- Control
    else
      v_p2_spell_key := 'expelliarmus'; -- Desarme
    end if;
  else
    -- Para PvP, obtenemos el hechizo del oponente de la tabla de turnos
  ...
  if v_duel.player_one_hp <= 0 or v_duel.player_two_hp <= 0 or v_duel.turn_number > 12 then
    v_winner_id := (case when v_duel.player_one_hp > v_duel.player_two_hp then v_duel.player_one else v_duel.player_two end);
