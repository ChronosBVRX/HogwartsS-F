...
-- Unificación de reglas de Duelos Mágicos v2
-- ==========================================

-- 1. Actualizar hsf_submit_duel_strategy
...
begin
  ...
  v_primary_spell_key := coalesce(p_actions->0->>'key', 'none');
...
-- 2. Motor de Resolución Avanzado
...
begin
  ...
  if v_p1_burn > 0 then v_p2_dmg := v_p2_dmg + 5; v_p1_burn := v_p1_burn - 1; end if;
  if v_p2_burn > 0 then v_p1_dmg := v_p1_dmg + 5; v_p2_burn := v_p2_burn - 1; end if;
  ...
  case v_p1_turn.stance
    ...
    when 'desperate' then if v_duel.player_one_hp < 25 then v_p1_stance_dmg_bonus := 6; else v_p1_stance_dmg_penalty := 3; end if;
    else null;
  end case;
  case v_p2_stance
    ...
    when 'desperate' then if v_duel.player_two_hp < 25 then v_p2_stance_dmg_bonus := 6; else v_p2_stance_dmg_penalty := 3; end if;
    else null;
  end case;
  ...
  for v_action in select * from jsonb_array_elements(v_p1_turn.actions) loop
    ...
    elsif v_spell_key = 'petrificus' then v_p1_dmg := v_p1_dmg + 15; v_p1_cost := v_p1_cost + 2; v_p1_fams := array_append(v_p1_fams, 'control'); v_p1_beats := v_p1_beats || '{defense}'::text[]; v_p1_cd := v_p1_cd || '{"petrificus": 2}'::jsonb; v_p2_cost := v_p2_cost + 1; v_p2_interrupted := true;
  ...
  for v_action in select * from jsonb_array_elements(v_p2_actions) loop
    ...
    elsif v_spell_key = 'petrificus' then v_p2_dmg := v_p2_dmg + 15; v_p2_cost := v_p2_cost + 2; v_p2_fams := array_append(v_p2_fams, 'control'); v_p2_beats := v_p2_beats || '{defense}'::text[]; v_p2_cd := v_p2_cd || '{"petrificus": 2}'::jsonb; v_p1_cost := v_p1_cost + 1; v_p1_interrupted := true;
  ...
  if v_p2_fams && v_p1_beats then 
    ...
    if (v_p1_fams && '{defense}'::text[]) and (v_p2_fams && '{heavy}'::text[]) then v_p1_gain := v_p1_gain + 1; end if;
  ...
  if v_p1_fams && v_p2_beats then 
    ...
    if (v_p2_fams && '{defense}'::text[]) and (v_p1_fams && '{heavy}'::text[]) then v_p2_gain := v_p2_gain + 1; end if;
    if (v_p2_fams && '{control}'::text[]) and (v_p1_fams && '{defense}'::text[]) then v_p1_blk := 0; end if;
  end if;

  -- 6. Debilidad
  if coalesce((v_meta->>'p1_weakness')::int, 0) > 0 then v_p1_dmg := greatest(0, v_p1_dmg - 8); end if;
  if coalesce((v_meta->>'p2_weakness')::int, 0) > 0 then v_p2_dmg := greatest(0, v_p2_dmg - 8); end if;

  -- 7. Cálculos Finales
  v_p1_final_blk := v_p1_blk + v_p1_stance_blk_bonus;
  v_p2_final_blk := v_p2_blk + v_p2_stance_blk_bonus;
  
  v_p1_pre_blk_dmg := greatest(0, v_p2_dmg + v_p2_stance_dmg_bonus + v_p2_strategy_bonus + v_p1_stance_vulnerability - v_p2_stance_dmg_penalty);
  v_p2_pre_blk_dmg := greatest(0, v_p1_dmg + v_p1_stance_dmg_bonus + v_p1_strategy_bonus + v_p2_stance_vulnerability - v_p1_stance_dmg_penalty);
  ...
  update hsf_duels set 
    player_one_hp = least(100, greatest(0, player_one_hp - v_p1_total_dmg + v_p1_heal)),
    player_two_hp = least(100, greatest(0, player_two_hp - v_p2_total_dmg + v_p2_heal)),
  ...
  values (p_duel_id, p_turn_number, 'turn_resolved', jsonb_build_object(
    ...
    'p1_damage', v_p1_total_dmg, 'p2_damage', v_p2_total_dmg,
    'p1_damage_dealt', v_p2_total_dmg, 'p2_damage_dealt', v_p1_total_dmg,
    ...
    'p1_stance_bonus', v_p1_stance_dmg_bonus, 'p2_stance_bonus', v_p2_stance_dmg_bonus,
    'p1_damage_taken_bonus', v_p1_stance_vulnerability, 'p1_damage_dealt_penalty', v_p1_stance_dmg_penalty,
    'p2_damage_taken_bonus', v_p2_stance_vulnerability, 'p2_damage_dealt_penalty', v_p2_stance_dmg_penalty,
