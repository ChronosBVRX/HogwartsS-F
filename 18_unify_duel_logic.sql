...
begin
  ...
  for v_action in select * from jsonb_array_elements(v_p1_turn.actions) loop
    ...
    elsif v_spell_key = 'confundus' then v_p1_dmg := v_p1_dmg + 6; v_p1_cost := v_p1_cost + 2; v_p1_cd := v_p1_cd || '{"confundus": 3}'::jsonb; v_p1_fams := array_append(v_p1_fams, 'control'); v_p1_beats := v_p1_beats || '{defense}'::text[]; v_p1_loses := v_p1_loses || '{counter}'::text[];
  ...
  for v_action in select * from jsonb_array_elements(v_p2_actions) loop
    ...
    elsif v_spell_key = 'confundus' then v_p2_dmg := v_p2_dmg + 6; v_p2_cost := v_p2_cost + 2; v_p2_cd := v_p2_cd || '{"confundus": 3}'::jsonb; v_p2_fams := array_append(v_p2_fams, 'control'); v_p2_beats := v_p2_beats || '{defense}'::text[]; v_p2_loses := v_p2_loses || '{counter}'::text[];
  ...
  -- 6. Cálculos Finales de Daño (CORREGIDO: Incluye Vulnerabilidad)
  v_p1_final_blk := v_p1_blk + v_p1_stance_blk_bonus;
  v_p2_final_blk := v_p2_blk + v_p2_stance_blk_bonus;
  
  -- P1 Recibe: (Daño Rival + Bonus Rival + Vulnerabilidad Propia - Penalización Rival - Bloqueo Propio)
  v_p1_total_dmg := greatest(0, v_p2_dmg + v_p2_stance_dmg_bonus + v_p2_strategy_bonus + v_p1_stance_vulnerability - v_p2_stance_dmg_penalty - v_p1_final_blk);
  -- P2 Recibe: (Daño Propio + Bonus Propio + Vulnerabilidad Rival - Penalización Propia - Bloqueo Rival)
  v_p2_total_dmg := greatest(0, v_p1_dmg + v_p1_stance_dmg_bonus + v_p1_strategy_bonus + v_p2_stance_vulnerability - v_p1_stance_dmg_penalty - v_p2_final_blk);
  ...
  -- 8. Evento Enriquecido (Separado Penalty vs Vulnerability)
  ...
  values (p_duel_id, p_turn_number, 'turn_resolved', jsonb_build_object(
    ...
    'p1_damage', v_p1_total_dmg, 
    'p2_damage', v_p2_total_dmg,
    'p1_damage_dealt', v_p2_total_dmg,
    'p2_damage_dealt', v_p1_total_dmg,
    ...
    'p1_damage_taken_bonus', v_p1_stance_vulnerability,
    'p1_damage_dealt_penalty', v_p1_stance_dmg_penalty,
    'p2_damage_taken_bonus', v_p2_stance_vulnerability,
    'p2_damage_dealt_penalty', v_p2_stance_dmg_penalty,
